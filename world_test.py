...
def reached_red(perception, distance, size_threshold=250, distance_threshold=200):
    red_blob = perception['blobs']['red']
    sensor = perception['proximity']['FrontC'] #FrontC does not work
    
    # Check if the red blob is detected and if its size exceeds the threshold
    if red_blob['seen'] and red_blob['size'] >= size_threshold and distance <= distance_threshold:
